...
def score(A, weights, method, alpha = None, beta = None):
    ...
    if method == 'gravitation':
        ...
        G = nx.from_numpy_matrix(A)
        max_boundary = 1/np.max(np.real(LA.eig(A)[0]))
        if alpha >= max_boundary:
            alpha = max_boundary - 0.1
        score = np.linalg.inv(np.identity(N) - alpha * A)-np.identity(N)
        # print(max_boundary)
    elif method == 'CN_cl' and weights == 'unweighted':
        G = nx.from_numpy_matrix(A)
        cl = np.array(list(nx.clustering(G).items()))[:, -1]
        degree = np.sum(A, 1)
        index = np.where((cl == 0) & (degree > 1))[0]
        cl[index] = 2/degree[index] / (degree[index]- 1)
        score = np.zeros((N, N))
        for i in range(N):
            score[i] = np.sum(A * A[i] * cl, 1)
    elif method == 'CN_cl' and weights == 'weighted':
        G = nx.from_numpy_matrix(A)
        cl = np.array(list(nx.clustering(G).items()))[:, -1]
        degree = np.sum(A, 1)
        index = np.where((cl == 0) & (degree > 1))[0]
        ...
        score = np.zeros((N, N))
        for  i in range(N):
            score[i] = np.sum(cl * (A[i] + A) * np.heaviside(A[i]*A, 0), 1)
...
def predictor(snapshot_num, weights, method, n, S, alpha=None, beta=None):
    ...
    "link prediction in the future snapshots"
    predict = 0
    
    A_actual, A_undirected, A_unweighted, node_list, M, _, _ = merge_weights(month_num-1, 1)
...
def beta_S(network_type, method, snapshot_num, beta_set, n, S, plot, alpha=None, beta=None):
    """TODO: Docstring for change_n.
    ...
    "directed"
    prediction = np.zeros((np.size(S), np.size(beta_set)))
    success_rate = np.zeros((np.size(S), np.size(beta_set)))
    for beta, i in zip(beta_set, range(np.size(beta_set))):
        beta = round(beta, 2)
        for s, j in zip(S, range(np.size(S))):
            t1 = time.time()
            prediction[j, i], success_rate[j, i] = predictor(snapshot_num, 'directed', method, n, s, beta=beta)
            t2 = time.time()
            print(beta, s, t2- t1)
        if plot == 'success':
            plt.plot(S, success_rate[:, i], linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label=f'$\\beta=${beta}')
            plt.ylabel('Recall of prediction', fontsize=fs)
        elif plot =='prediction':
            plt.plot(S, prediction[:, i], linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label=f'$\\beta=${beta}')
            plt.ylabel('Successfully predicted edges', fontsize=fs)
    "unweighted"
    prediction = np.zeros((np.size(S)))
    success_rate = np.zeros((np.size(S)))
    for s, j in zip(S, range(np.size(S))):
        t1 = time.time()
        prediction[j], success_rate[j] = predictor(snapshot_num, 'unweighted', method, n, s)
        t2 = time.time()
        print('unweighted', s, t2 - t1)
    if plot == 'success':
        plt.plot(S, success_rate, linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label='unweighted')
        plt.ylabel('Recall of prediction', fontsize=fs)
    elif plot =='prediction':
        plt.plot(S, prediction, linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label='unweighted')
        plt.ylabel('Successfully predicted edges', fontsize=fs)

    "weighted"
    for s, j in zip(S, range(np.size(S))):
        t1 = time.time()
        prediction[j], success_rate[j] = predictor(snapshot_num, 'weighted', method, n, s)
        t2 = time.time()
        print('weighted', s, t2 - t1)
    if plot == 'success':
        plt.plot(S, success_rate, linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label='weighted')
        plt.ylabel('Recall of prediction', fontsize=fs)
    elif plot =='prediction':
        plt.plot(S, prediction, linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label='weighted')

        plt.ylabel('Successfully predicted edges', fontsize=fs)

    plt.xlabel('Memory effect S', fontsize=fs)
    plt.subplots_adjust(left=0.15, right=0.90, wspace=0.25, hspace=0.25, bottom=0.15, top=0.98)
    plt.xticks(fontsize=ticksize)
    plt.yticks(fontsize=ticksize)
    plt.legend(fontsize=legendsize)
...
def alpha_n(network_type, method, snapshot_num, alpha_set, n, S, plot):
    """TODO: Docstring for change_n.
    ...
    "weighted"
    prediction = np.zeros((np.size(n), np.size(alpha_set)))
    success_rate = np.zeros((np.size(n), np.size(alpha_set)))
    for alpha, i in zip(alpha_set, range(np.size(alpha_set))):
        alpha = round(alpha, 3)
        for portion, j in zip(n, range(np.size(n))):
            t1 = time.time()
            prediction[j, i], success_rate[j, i] = predictor(snapshot_num, 'weighted', method, portion, S, alpha=alpha)
            t2 = time.time()
            print(alpha, portion, t2- t1)
        if plot == 'success':
            plt.plot(n, success_rate[:, i], linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label=f'$\\alpha=${alpha}')
            plt.ylabel('Recall of prediction', fontsize=fs)
        elif plot =='prediction':
            plt.plot(n, prediction[:, i], linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label=f'$\\alpha=${alpha}')
            plt.ylabel('Successfully predicted edges', fontsize=fs)
...
def cl_n(network_type, method, weights, snapshot_num, n, S, plot):
    """TODO: Docstring for change_n.
    ...
    for portion, j in zip(n, range(np.size(n))):
        t1 = time.time()
        prediction[j], success_rate[j] = predictor(snapshot_num, weights, method, portion, S)
        t2 = time.time()
        print(portion, t2- t1)
    if plot == 'success':
        plt.plot(n, success_rate, linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker))
        plt.ylabel('Recall of prediction', fontsize=fs)
    elif plot =='prediction':
        plt.plot(n, prediction, linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker))
        plt.ylabel('Successfully predicted edges', fontsize=fs)

    for portion, j in zip(n, range(np.size(n))):
        t1 = time.time()
        prediction[j], success_rate[j] = predictor(snapshot_num, 'weighted', 'CN', portion, S)
        t2 = time.time()
        print('weighted', portion, t2 - t1)
    if plot == 'success':
        plt.plot(n, success_rate, linestyle = next(linestyle), linewidth = next(linewidth), alpha = alpha_c, marker = next(marker), label='weighted')
...
def ml_features(A):
    ...
    feature = np.vstack((np.ravel(CN), np.ravel(A_out), np.ravel(A_in), np.ravel(Di_out), np.ravel(Dj_out), np.ravel(Di_in), np.ravel(Dj_in))).transpose()
...
def ml_logistic_three(a1, a2, a3, k):
    ...
    """
    feature1, A1 = ml_features(a1)
    feature2, A2 = ml_features(a2)
    feature3, A3 = ml_features(a3)

    Y1_train = np.where(A2 - A1 == 1)[0]
    Y1_train_num = np.size(Y1_train)
    Y0_train = np.where(A2 == 0)[0]
    ...
    return truth, prediction, probability, accuracy, precision, recall, fscore

def ml_logistic_four(a1, a2_modify, a2, a3, k, threshold_positive):
    """TODO: Docstring for ml_logistic.
    ...
    """
    feature1, A1 = ml_features(a1)
    feature2_modify, A2_modify = ml_features(a2_modify)
    feature2, A2 = ml_features(a2)
    feature3, A3 = ml_features(a3)

    Y1_train = np.where(A2_modify - A1 == 1)[0]
    Y1_train_num = np.size(Y1_train)
    Y0_train = np.where(A2_modify == 0)[0]
    Y0_train_num = np.size(Y0_train)
    X_train = np.vstack((feature1[Y1_train], feature1[Y0_train]))
    Y_train = np.hstack((np.ones(Y1_train_num), np.zeros(Y0_train_num)))
    ...
    Y1_test = np.where(A3-A2 == 1)[0]
    Y0_test = np.where(A3 == 0)[0]
    X_test = np.vstack((feature2_modify[Y1_test], feature2_modify[Y0_test]))
    Y_test = np.hstack((np.ones(np.size(Y1_test)), np.zeros(np.size(Y0_test))))
    ...
    probability = clf.predict_proba(X_test[:, :])
    prediction = clf.predict(X_test[:, :])  # default threshold is 0.5
    #prediction[np.where(A2_modify - A2 == 1)[0]] = 1
    prediction = (probability[:, 1] >= threshold_positive).astype(bool)
    tru_pos = np.where(Y_test == 1)[0]
    pre_pos = np.where(prediction == 1)[0]
    common = np.array(list(set(tru_pos).intersection(pre_pos))) 
    tp = np.size(common)
...
def hybrid(network_type, S, snapshot_num, train_num, test_num, k, norm, add_num, remove_threshold, threshold_positive):
    """TODO: Docstring for hybrid.
    ...
    for i in range(train_num):
        if i == 0:
            A_actual, A_undirected, A_unweighted, node_list, M, a1, _ = merge_weights(snapshot_num + i, S)
        else:
            a1 = modify
        A_actual, A_undirected, A_unweighted, node_list, M, a2, _ = merge_weights(snapshot_num+i+1, S)
        truth, prediction, probability, accuracy, precision, recall, fscore = ml_logistic_two(a1, a2, k)
        prediction = (probability[:, 1] > np.sort(probability[:, 1])[::-1][add_num]).astype(bool)
        a2_ravel = np.ravel(a2)
        d_max = np.max(a2_ravel) 
        d_mean = np.mean(a2_ravel[a2_ravel>0])
        diff = prediction - np.heaviside(a2_ravel, 0) 
        add = probability[:, 1] * d_mean / norm * (diff == 1).astype(bool)
        prediction_non = (probability[:, 1] < np.sort(probability[:, 1])[add_num]).astype(bool)
        remove = (np.heaviside(a2_ravel, 0) + prediction_non == 2).astype(bool) * (a2_ravel<remove_threshold * d_mean).astype(bool) * a2_ravel
        modify = a2_ravel + add - remove
        modify = modify.reshape(N_total, N_total)
    
    A_actual, A_undirected, A_unweighted, node_list, M, a1, _ = merge_weights(snapshot_num, S)
    A_actual, A_undirected, A_unweighted, node_list, M, a2, _ = merge_weights(snapshot_num+train_num, S)
    A_actual, A_undirected, A_unweighted, node_list, M, a3, _ = merge_weights(snapshot_num + train_num + test_num, S)
    truth, prediction, probability, accuracy, precision, recall, fscore = ml_logistic_four(a1, modify, a2, a3, k, threshold_positive)
    return accuracy, precision, recall, fscore

def ml_fscore_threshold(network_type, S, snapshot_num, train_num, test_num, k, norm, add_threshold, remove_threshold, threshold_positive):
    """TODO: Docstring for ml_plot.
    ...
    """
    for threshold in threshold_positive:
        ...
        fscore = np.zeros(np.size(k))
        for k_value, i in zip(k, range(np.size(k))):
            accuracy[i], precision[i], recall[i], fscore[i] = hybrid(network_type, S, snapshot_num, train_num, test_num, k_value, norm, add_threshold, remove_threshold, threshold)

        plt.plot(k, fscore, linewidth = 3, alpha = alpha_c, marker = next(marker), label=f'threshold={threshold}')
    ...
    plt.yticks(fontsize=ticksize)
    plt.legend(loc='best', fontsize=legendsize)
    plt.savefig(f'../report/caviar_ml_logistic_snap{snapshot_num}_threshold.png')
    plt.close()
    #plt.show()

def ml_hybrid_k(network_type, S, snapshot_num, train_num, test_num, k, norm, add_threshold, remove_threshold, threshold_positive):
    """TODO: Docstring for ml_plot.
    ...
    fscore = np.zeros(np.size(k))
    for k_value, i in zip(k, range(np.size(k))):
        accuracy[i], precision[i], recall[i], fscore[i] = hybrid(network_type, S, snapshot_num, train_num, test_num, k_value, norm, add_threshold, remove_threshold, threshold_positive)
    ...
    plt.yticks(fontsize=ticksize)
    plt.legend(loc='best', fontsize=legendsize)
    plt.savefig(f'../report/caviar_ml_logistic_snap{snapshot_num}_hybrid.png')
    # plt.close()
    #plt.show()

def ml_fscore_add(network_type, S, snapshot_num, train_num, test_num, k, norm, add_num, remove_threshold, threshold_positive):
    """TODO: Docstring for ml_plot.
    ...
    for addnum in add_num:
        ...
        fscore = np.zeros(np.size(k))
        for k_value, i in zip(k, range(np.size(k))):
            accuracy[i], precision[i], recall[i], fscore[i] = hybrid(network_type, S, snapshot_num, train_num, test_num, k_value, norm, addnum, remove_threshold, threshold_positive)

        plt.plot(k, fscore, linewidth = 3, alpha = alpha_c, marker = next(marker), label=f'add={addnum}')
    ...
    plt.yticks(fontsize=ticksize)
    plt.legend(loc='best', fontsize=legendsize)
    plt.savefig(f'../report/caviar_ml_logistic_snap{snapshot_num}_add.png')
    plt.close()
    #plt.show()

def network_property(network_type, snapshot_num):
    """TODO: Docstring for network_property.
...
n = 0.1
n = np.arange(0.1, 0.5, 0.1)
beta_set = np.arange(0, 1.1, 0.2)
alpha_set = np.arange(0.001, 0.018, 0.001)
S = 'inf'
S = 1
S_set = np.array([0, 0.1, 0.2, 0.5, 1, 5])
snapshot_num = 0
realization = 1
plot_range = np.arange(8)
snapshot_range = np.arange(8)
k = np.arange(0.51, 5, 0.1)
k = np.arange(5, 10, 0.5)
k[0] = 1
k = np.arange(1, 30, 1)
feature_index = 4
noise = 0
...
select_num = 10
norm = 1
add_threshold = 1 - 1e-15
remove_threshold = 1e-15
add_threshold = 0.98
add_num = 0
remove_threshold = 0.1
train_num = 1
test_num = 4
threshold_positive = np.array([ 0.4, 0.5, 0.6, 0.7, 0.8, 0.9])
add_num = np.array([0,5,10,20])
...
#for snapshot_num in range(1, 4):
    ...
    #ml_plot_k(network_type, S, snapshot_num, k)
    #accuracy, precision, recall, fscore = hybrid(network_type, S, snapshot_num, train_num, test_num, k, norm, add_threshold, remove_threshold)
    #test_num = 10 - snapshot_num - train_num
    #ml_hybrid_k(network_type, S, snapshot_num, train_num, test_num, k, norm, add_num, remove_threshold, 0.5)
    #ml_fscore_add(network_type, S, snapshot_num, train_num, test_num, k, norm, add_num, remove_threshold, 0.5)
    #ml_fscore_threshold(network_type, S, snapshot_num, train_num, test_num, k, norm, add_num, remove_threshold, threshold_positive)
...
# prediction, success = beta_S(network_type, method, snapshot_num, beta_set, n, S_set, plot)
prediction, success = cl_n(network_type, method, weights,  snapshot_num, n, S, plot)
#prediction, success = alpha_n(network_type, method, snapshot_num, alpha_set, n, S, plot)
