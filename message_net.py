...
index_separate = [np.where(month == i)[0][-1] for i in range(4, 11)]
...
def predictor(snapshot_num, weights, method, n, S, alpha=None, beta=None):
    ...
    for i in range(min(20, 7-snapshot_num)):
...
def new_edge_pattern(S):
    ...
    New_edge = []
    Node = []
    A = np.zeros((7, N_total, N_total))
    count_all = np.zeros((7, 3))
    count_percentage = np.zeros((7, 3))
    with_cn = np.zeros((7, 2))
    M_num_all = np.zeros((7, 3))
    for i in range(7):
        snapshot_num = i 
...
def plot_motif(count_all, count_percentage, with_cn, M_num_all, plot_range, plot_type):
    ...
    if plot_type == 'with_cn':
        plt.plot(np.arange(7) + 1, with_cn[:, 0], 'o-')
